export const guideFirstUsePages = {
  'premiere-utilisation-cafetiere-italienne': {
    title: 'Première utilisation d’une cafetière italienne : que faire ?',
    description: 'Premier lavage, cafés à jeter et précautions : les étapes à vérifier avant d’utiliser une cafetière italienne pour la première fois.',
    canonical: '/guides/premiere-utilisation-cafetiere-italienne/',
    body: `
<section class="page-hero guide-hero">
  <div class="container">
    <div class="breadcrumbs"><a href="/">Accueil</a> · <a href="/guides/">Guides</a> · Première utilisation d’une cafetière italienne</div>
    <span class="eyebrow">Mise en service</span>
    <h1>Première utilisation d’une cafetière italienne : que faire ?</h1>
    <p>La première utilisation dépend du modèle. Pour la Moka Express, Bialetti recommande de laver d’abord toutes les pièces à l’eau chaude, puis de préparer et jeter les trois premières infusions. Pour une autre moka, vérifiez la notice avant de reprendre cette routine telle quelle.</p>
  </div>
</section>

<div class="container template-grid guide-layout">
  <article class="guide-article content-main">
    <section class="guide-answer">
      <span class="eyebrow">Réponse courte</span>
      <h2>Que faire avant de boire le premier café ?</h2>
      <p>Commencez par identifier le modèle et lire ses instructions de mise en service. Sur la <strong>Bialetti Moka Express</strong>, la marque donne trois consignes très claires :</p>
      <ol class="decision-list">
        <li><strong>Laver toutes les pièces à l’eau chaude</strong> avant la première préparation.</li>
        <li><strong>Réaliser trois infusions et les jeter</strong> avant de boire le café préparé avec la cafetière.</li>
        <li><strong>Ne pas utiliser la poignée pour dévisser</strong> la cafetière, afin d’éviter de l’endommager.</li>
      </ol>
      <p>Ces consignes sont documentées pour la Moka Express. Elles ne doivent pas être transformées en règle universelle pour toutes les cafetières italiennes.</p>
    </section>

    <section>
      <h2>1. Identifiez d’abord votre modèle</h2>
      <p>« Cafetière italienne » désigne une famille d’appareils, pas une notice unique. Deux moka proches visuellement peuvent différer par le matériau, le système de fermeture, la compatibilité avec la plaque ou les recommandations de nettoyage.</p>
      <p>Avant toute première chauffe, repérez donc la marque et la référence exacte. Si la notice du fabricant est disponible, elle prime sur les conseils génériques trouvés en ligne.</p>
      <div class="guide-callout"><strong>Pourquoi c’est important :</strong> le nombre de préparations à jeter, les produits de nettoyage autorisés ou le mode de fermeture ne sont pas des règles que l’on peut généraliser sans vérifier le modèle.</div>
    </section>

    <section>
      <h2>2. Faites le premier lavage demandé par le fabricant</h2>
      <p>Pour la Moka Express, Bialetti recommande de laver toutes les pièces à l’eau chaude avant la première utilisation. La marque ne demande pas, dans cette consigne de mise en service, d’ajouter une procédure complexe ou un produit spécifique.</p>
      <p>Le point important est donc de ne pas improviser un « nettoyage en profondeur » avec du vinaigre, du bicarbonate, un détergent ou le lave-vaisselle si le fabricant de votre modèle ne le prévoit pas.</p>
      <p>Après cette première mise en service, l’entretien courant devient un sujet séparé : <a class="text-link" href="/guides/nettoyer-cafetiere-italienne/">comment nettoyer une cafetière italienne</a>.</p>
    </section>

    <section>
      <h2>3. Faut-il jeter les premiers cafés ?</h2>
      <p><strong>Oui pour la Bialetti Moka Express :</strong> Bialetti indique de jeter les trois premières infusions. Il s’agit donc d’une consigne fabricant, pas d’une tradition que nous extrapolons.</p>
      <p>Pour une autre marque ou un autre modèle, la bonne réponse est : vérifiez la notice. Si elle demande une ou plusieurs préparations de rinçage ou de mise en service, suivez cette consigne ; si elle prévoit autre chose, ne copiez pas automatiquement la règle des trois cafés.</p>
      <div class="guide-callout"><strong>À retenir :</strong> « jeter trois cafés » est une réponse exacte pour la Moka Express, pas une preuve que toutes les moka doivent être traitées de la même manière.</div>
    </section>

    <section>
      <h2>4. Préparez ces infusions comme une utilisation normale</h2>
      <p>Pour la Moka Express, Bialetti décrit ensuite la préparation classique : eau dans la base sous la soupape de sécurité, filtre en place, café moulu sans le compacter, fermeture correcte, puis chauffe douce à moyenne.</p>
      <p>Vous n’avez donc pas besoin d’inventer une recette spéciale pour les préparations à jeter. Utilisez la procédure prévue pour la cafetière, puis éliminez le café si la consigne de première utilisation l’exige.</p>
      <p>Le mode d’emploi complet est détaillé ici : <a class="text-link" href="/guides/comment-utiliser-cafetiere-italienne/">comment utiliser une cafetière italienne</a>.</p>
    </section>

    <section>
      <h2>5. Dévissez la cafetière sans forcer sur la poignée</h2>
      <p>Bialetti insiste sur un point spécifique lors de la première utilisation de la Moka Express : ne pas utiliser la poignée comme levier pour dévisser la cafetière, car elle peut se casser.</p>
      <p>Laissez la cafetière refroidir suffisamment pour pouvoir la manipuler en sécurité, puis saisissez les parties du corps prévues pour le vissage et le dévissage plutôt que de tordre la poignée.</p>
      <p>Cette précaution est simple, mais elle évite de transformer une mise en service banale en problème mécanique dès les premières utilisations.</p>
    </section>

    <section>
      <h2>6. Quand votre moka est-elle prête pour un usage normal ?</h2>
      <p>Une fois les étapes de mise en service indiquées par le fabricant terminées, vous pouvez passer à la routine normale. Pour une Moka Express neuve, cela signifie notamment avoir effectué le lavage initial et les trois préparations à jeter demandées par Bialetti.</p>
      <p>À partir de là, la qualité du café dépend davantage des variables de recette et d’usage : quantité de café, mouture, café choisi et maîtrise de la chauffe.</p>
      <div class="actions">
        <a class="btn btn-secondary" href="/guides/dosage-cafe-cafetiere-italienne/">Voir le dosage</a>
        <a class="btn btn-secondary" href="/guides/mouture-cafetiere-italienne/">Choisir la mouture</a>
      </div>
    </section>

    <section>
      <h2>Ce qu’il vaut mieux ne pas généraliser</h2>
      <ul class="guide-list">
        <li><strong>Le nombre de cafés à jeter.</strong> Trois est la consigne Bialetti pour la Moka Express, pas une loi universelle.</li>
        <li><strong>Le produit de nettoyage.</strong> N’ajoutez pas savon, vinaigre ou autre produit par réflexe sans vérifier les instructions du modèle.</li>
        <li><strong>Le lave-vaisselle.</strong> La compatibilité dépend du matériau et de la finition ; la notice doit trancher.</li>
        <li><strong>Le « culottage ».</strong> Ne remplacez pas une consigne fabricant par une croyance selon laquelle une moka ne devrait jamais être lavée.</li>
        <li><strong>La compatibilité plaque.</strong> Une cafetière neuve n’est pas automatiquement compatible avec l’induction.</li>
      </ul>
    </section>

    <section class="guide-next">
      <span class="eyebrow">Après la mise en service</span>
      <h2>Passez maintenant au mode d’emploi quotidien</h2>
      <p>La première utilisation est une étape ponctuelle. Une fois terminée, gardez une routine simple et conforme à votre modèle pour les préparations suivantes.</p>
      <a class="btn btn-primary" href="/guides/comment-utiliser-cafetiere-italienne/">Voir comment utiliser la moka →</a>
    </section>

    <section class="guide-sources">
      <h2 id="sources">Sources vérifiées</h2>
      <p>Les consignes spécifiques à la Moka Express ci-dessus proviennent directement de Bialetti. Pour une autre cafetière, consultez la notice du modèle exact.</p>
      <ul>
        <li><a href="https://bialetti-cookware.zendesk.com/hc/fr/articles/5416180168210-Comment-utiliser-la-Moka-Express-pour-la-premi%C3%A8re-fois">Bialetti — Comment utiliser la Moka Express pour la première fois ?</a></li>
        <li><a href="https://bialetti-cookware.zendesk.com/hc/fr/articles/5416235346322-Comment-utiliser-la-Moka-Express">Bialetti — Comment utiliser la Moka Express ?</a></li>
      </ul>
      <p class="source-date">Dernière vérification des sources : 13 septembre 2026.</p>
    </section>
  </article>

  <aside>
    <div class="sidebar-card guide-sidebar">
      <span class="eyebrow">Moka Express</span>
      <h3>Première mise en service</h3>
      <ol>
        <li>Laver à l’eau chaude</li>
        <li>Faire 3 préparations</li>
        <li>Jeter ces 3 cafés</li>
        <li>Ne pas dévisser par la poignée</li>
      </ol>
      <p>Pour une autre moka, vérifiez la notice avant d’appliquer cette séquence.</p>
      <a class="sidebar-link" href="/guides/comment-utiliser-cafetiere-italienne/">Mode d’emploi quotidien →</a>
    </div>
  </aside>
</div>`
  }
};
